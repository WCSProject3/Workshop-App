...
const MyWorkshopDetails = ({ workshop, reachedLimit }) => {
    ...
    useEffect(() => {
        calcPercentage(workshop.enrolled_ateendees, workshop.room_capacity)
    },[workshops])
   ...
   const calcPercentage = (enrolled, room_capacity) => {
    ...
    return (
        <div className={isEnrolled ? "myWorkshop-details-bg-enrolled" : "myWorkshop-details"}>
            <div className="myWorkshop-basic-info">
                <div className="myWorkshop-basic-info-left">
                    <p className={isEnrolled ? "myWorkshop-details-title-enroled" :"myWorkshop-details-title"}>{workshop.title}</p>
                    <div className="myWorkshop-details-date">
                        <p className={isEnrolled ? "enroled" :"myWorkshop-details-day"}>{`${workshop.workshop_month} ${day}`}</p>
                        <div className={isEnrolled ? "dot-enroled" :"dot"} />
                        <p className={isEnrolled ? "enroled" : "myWorkshop-details-hour"}>{`Time: ${workshop.starting_hour.substring(0,5)} - ${workshop.ending_hour.substring(0,5)}`}</p>
                    </div>
                </div>
                <div className="myWorkshop-basic-info-right">
                    <div className="myWorkshops-room-capacity">
                        <div className="myWorkshops-room-capacity-info">
                            <p className={isEnrolled ? "enroled" :"myWorkshops-room-capacity-title"}>Room Capacity</p>
                            {percentage !== null && 
                                <p className={isEnrolled ? "enroled" :"myWorkshops-room-capacity-title"}>{percentage}%</p>}
                        </div>
                        <div className="progress-bar enroled">
                            {percentage !== null && <div className="progress-bar-fill" style={({width: `${percentage}%`})} />}
                        </div>
                    </div>
                    ...
                    :
                    <button className="myWorkshop-handle-expanded-open" onClick={() => handleIsExpanded(true)}>show more</button>}
                    
                    </div>
                </div>
            </div>
            <div className={isExpanded ? (isEnrolled ? "myWorkshop-show-enroled" : "myWorkshop-show-not-enroled") : "myWorkshop-hide"}>
